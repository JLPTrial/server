...
def _firebase_identity(
    uid: str, email: str, name: str | None = None
) -> FirebaseIdentity:
    return FirebaseIdentity(uid=uid, email=email, name=name)


def test_firebase_signup_creates_local_user(
    ...
    monkeypatch.setattr(
        ...
        lambda uid_token: _firebase_identity(
            "firebase-uid-123", "fulano@jlptrial.com", "Fulano"
        ),
    )
    monkeypatch.setattr(
        login_routes,
...
def test_firebase_login_auto_registers_unknown_user(
    ...
    monkeypatch.setattr(
        ...
        lambda uid_token: _firebase_identity(
            "firebase-uid-456", "missing@jlptrial.com", "Missing"
        ),
    )
    monkeypatch.setattr(
        login_routes,
...
def test_firebase_login_sets_session_cookie(
    client: TestClient, db, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "SECURE_REQUEST", True)
    ...
    monkeypatch.setattr(
        "src.api.services.login.resolve_login_identity",
        lambda uid_token: _firebase_identity(
            "firebase-uid-789", "active@jlptrial.com", "Active User"
        ),
    )
    monkeypatch.setattr(
        login_routes,
...
def test_firebase_refresh_reads_session_cookie(
    client: TestClient, db, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "SECURE_REQUEST", True)
    monkeypatch.setattr(
        "src.api.services.login.resolve_session_identity",
        lambda session_cookie: _firebase_identity(
            "firebase-uid-111", "refresh@jlptrial.com", "Refresh User"
        ),
    )
